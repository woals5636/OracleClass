...
-- SCOTT에서 기본 SELECT문을 익히기 위해 테이블을 생성.
-- 1) sql plus 실행해서 SYS 계정으로 접속.
-- 2) c:\\oraclexe 폴더       scott.sql 파일 검색
SELECT * FROM tabs;
-- 오라클은 사용자 계정(scott)을 생성하면 동일한 이름(scott)으로
-- DB 객체를 생성한다.  => 스키마(Schema)
DESC EMP;
DESCRIBE EMP;
-- Emp 테이블에 사원 정보 조회
-- DQL : SELECT문
WITH          --1
SELECT        --6       DISTINCT, ALL, AS 
FROM          --2
WHERE         --3
GROUP BY      --4
HAVING        --5
...
-- 사원번호, 사원명, 잡 컬럼 모두가 중복되었는지 확인하여 출력
